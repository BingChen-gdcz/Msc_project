...
'''
def gen_batch(data_dir,dicts,max_len=2500,batch_size=16,des_embed=False,version='mimic3'):
    print('Load data ... ')
...
def load_all_dict(label_set='full',version='mimic3'):
    dicts = {}
    c2ind,ind2c = all_codes_list(label_set,version=version)
    w2ind,ind2w = all_vocab_list(version=version)
    des_inds_dict = load_des_vector(version=version)
    dicts['c2ind'] = c2ind
    dicts['ind2c'] = ind2c
    dicts['w2ind'] = w2ind
    dicts['ind2w'] = ind2w
...
def all_vocab_list(version='mimic3'):
    if version == 'mimic3':
        vocabs_dir = os.path.join(os.pardir,'mimicdata/mimic3/vocab.csv')
    else:
        vocabs_dir = os.path.join(os.pardir,'mimicdata/mimic2/vocab.csv')
    vocabs = set()
    '''
...
def all_codes_list(label_set='full',version='mimic3'):
    if version == 'mimic2':
        MIMIC_2 = os.path.join(os.pardir,'mimicdata/mimic2')
        ind2c = defaultdict(str)
        codes = set()
        with open('%s/proc_dsums.csv'%MIMIC_2,'r') as f:
            r = csv.reader(f)
            next(r)
            for row in r:
...
def load_embedding_vector(version='mimic3'):
    print(version)
    if version == 'mimic3':
        vocabs_matrix_dir = os.path.join(os.pardir,'mimicdata/mimic3/processed_full.embed')#vocab_matrix.w2v')##
    else:
        vocabs_matrix_dir = os.path.join(os.pardir,'mimicdata/mimic2/processed_full.embed')
    embed_matrix = []
...
def load_des_vector(version='mimic3'):
    ''':return {code: des_inds}'''

    w2ind,ind2w = all_vocab_list(version=version)

    icd_des_dic=load_description(version=version) #get {code, [des_words_list]}
    des_inds_dict = {}
